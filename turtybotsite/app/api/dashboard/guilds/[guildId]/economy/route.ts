...
import {isDashboardApiError, updateDashboardEconomySettings} from "@/lib/dashboard-api";

export async function PUT(request: NextRequest, {params}: {params: Promise<{guildId: string}>}) {
    const session = await getCurrentSession();
    if (!session) {
        return NextResponse.json({
            message: "You must be signed in to update economy settings."
        }, {status: 401});
    }
    ...
    try {
        const updated = await updateDashboardEconomySettings(guildId, payload);
