...
import {fetchDashboardSuggestions, isDashboardApiError} from "@/lib/dashboard-api";
...
export async function GET(request: NextRequest, {params}: {params: Promise<{guildId: string}>}) {
    ...
    if (!session) {
        return NextResponse.json({
            message: "You must be signed in to view suggestions."
        }, {status: 401});
    }
    ...
    try {
        return NextResponse.json(await fetchDashboardSuggestions(guildId, page, pageSize));
    } catch (error) {
