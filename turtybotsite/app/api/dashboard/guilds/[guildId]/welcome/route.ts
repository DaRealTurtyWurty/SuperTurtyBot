...
import {isDashboardApiError, updateDashboardWelcomeSettings} from "@/lib/dashboard-api";

export async function PUT(request: NextRequest, {params}: {params: Promise<{guildId: string}>}) {
    const session = await getCurrentSession();
    if (!session) {
        return NextResponse.json({
            message: "You must be signed in to update welcome settings."
        }, {status: 401});
    }
    ...
    try {
        const updated = await updateDashboardWelcomeSettings(guildId, payload);
