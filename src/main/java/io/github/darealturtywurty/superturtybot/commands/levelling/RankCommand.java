...
public class RankCommand extends CoreCommand {
    ...
    protected void runSlash(SlashCommandInteractionEvent event) {
        ...
        final int level = TurtyBotDatabase.LEVELS.getLevel(member);
        final int xp = TurtyBotDatabase.LEVELS.getXP(member);
        ...
        final File card = makeRankCard(member, new RankCard(member), level, xp, forLevel, percentage);
        event.deferReply().addFile(card).mentionRepliedUser(false).queue();
    }
    
    @Nullable
    private File makeRankCard(final Member member, RankCard card, int level, int xp, int nextLevelXp, float xpPercent) {
        ...
        try {
            ...
            if (!card.backgroundImage.isBlank()) {
                background = ImageIO
                    .read(TurtyBot.class.getResourceAsStream("/levels/background/" + card.backgroundImage + ".png"));
            } else {
            ...
            if (!card.outlineImage.isBlank()) {
                outlineImg = ImageIO
                    .read(TurtyBot.class.getResourceAsStream("/levels/outline/" + card.outlineImage + ".png"));
            } else {
            ...
            int rank = TurtyBotDatabase.LEVELS.getRank(member);
            ...
            // XP Bar
            if (!card.xpOutlineImage.isBlank()) {
                final BufferedImage xpBarOutline = makeRoundedCorner(ImageIO
                    .read(TurtyBot.class.getResourceAsStream("/levels/xpOutline/" + card.xpOutlineImage + ".png")), 20);
            ...
            if (!card.xpEmptyImage.isBlank()) {
                final BufferedImage xpBarEmpty = makeRoundedCorner(
                    ImageIO.read(TurtyBot.class.getResourceAsStream("/levels/xpEmpty/" + card.xpEmptyImage + ".png")),
                    20);
            ...
            if (!card.xpFillImage.isBlank()) {
                final BufferedImage xpBarFill = makeRoundedCorner(
                    ImageIO.read(TurtyBot.class.getResourceAsStream("/levels/xpFill/" + card.xpFillImage + ".png")),
                    20);
            ...
            if (!card.avatarOutlineImage.isBlank()) {
                ...
                avatarOutGraphics.drawImage(
                    ImageIO.read(TurtyBot.class
                        .getResourceAsStream("/levels/avatarOutline/" + card.avatarOutlineImage + ".png")),
