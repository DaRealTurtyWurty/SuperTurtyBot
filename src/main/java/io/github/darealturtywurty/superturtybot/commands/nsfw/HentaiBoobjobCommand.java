...
public class HentaiBoobjobCommand extends NSFWCommand {
    public HentaiBoobjobCommand() {
        super(NSFWCategory.FAKE);
    }

    @Override
    public String getDescription() {
        return "Grabs some hentai boobjob images and other media";
    }
    ...
    protected void runNormalMessage(MessageReceivedEvent event) {
        ...
        try (var response = Constants.HTTP_CLIENT.newCall(request).execute()) {
            ...
            event.getChannel().sendMessage(
                "There was an issue retrieving some hentai boobjobs. Please report the following error to the bot owner:\n"
                    + strBuilder.toString())
