...
public class HentaiBoobsCommand extends NSFWCommand {
    public HentaiBoobsCommand() {
        super(NSFWCategory.FAKE);
    }

    @Override
    public String getDescription() {
        return "Grabs some hentai boob images and other media";
    }
    ...
    protected void runNormalMessage(MessageReceivedEvent event) {
        ...
        try (var response = Constants.HTTP_CLIENT.newCall(request).execute()) {
            ...
            event.getChannel().sendMessage(
                "There was an issue retrieving some hentai boobs. Please report the following error to the bot owner:\n"
                    + strBuilder.toString())
