...
public class HentaiAssCommand extends NSFWCommand {
    public HentaiAssCommand() {
        super(NSFWCategory.FAKE);
    }

    @Override
    public String getDescription() {
        return "Grabs some hentai ass images and other media";
    }
    ...
    protected void runNormalMessage(MessageReceivedEvent event) {
        ...
        final var request = new Request.Builder().url("https://nekobot.xyz/api/image?type=hass").build();
        try (var response = Constants.HTTP_CLIENT.newCall(request).execute()) {
            ...
            event.getChannel().sendMessage(
                "There was an issue retrieving some hentai ass. Please report the following error to the bot owner:\n"
                    + strBuilder.toString())
