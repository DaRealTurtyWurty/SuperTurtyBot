...
public class ServerInfoCommand extends CoreCommand {
    ...
    private static EmbedBuilder createEmbed(Guild guild) {
        ...
        embed.addField("Emotes", guild.getEmotes().size() + "", true);
        ...
        embed.addField("Features", formatFeatures(guild.getFeatures().toArray(new String[0])), false);
    ...
    private static String formatFeatures(String... features) {
        final var featureBuilder = new StringBuilder();
        for (final String feature : features) {
            featureBuilder.append(StringUtils.upperSnakeToSpacedPascal(feature) + ", ");
        }
        
        featureBuilder.delete(featureBuilder.length() - 2, featureBuilder.length());
