    ...
    public List<SubcommandData> createSubcommands() {
        return List.of(
                new SubcommandData("create", "Creates a role selection menu")
                        .addOption(OptionType.CHANNEL, "channel", "The channel to create this role selection menu in",
                                true)
                        .addOptions(new OptionData(OptionType.STRING, "title", "The title for this role selection menu",
                                true)
                        ...
                        .addOptions(
                                new OptionData(OptionType.STRING, "description", "The description for this role", true)
                                        .setRequiredLength(1, 1028))
                ...
                new SubcommandData("delete", "Deletes an existing role selection menu").addOption(OptionType.STRING,
                        "messageURL", "The URL of the message that contains the role selection menu", true, true),
                new SubcommandData("remove", "Removes a role from an existing role selection menu")
                        .addOption(OptionType.STRING, "messageURL",
                                "The URL of the message that contains the role selection menu", true,
                                true)
    ...
    protected void runSlash(SlashCommandInteractionEvent event) {
        ...
        switch (subcommand) {
            case "create" -> {
                ...
                channel.sendMessageEmbeds(embed.build()).queue(msg -> {
                    msg.editMessageComponents(ActionRow.of(SelectMenu.create("role-selection-" + msg.getId())
                                                                     .addOption(role.getName(), role.getId(),
                                                                             Emoji.fromFormatted(emoji)).build()))
                       .queue();
            ...
            case "add" -> {
                ...
                for (final Component component : message.getComponents()) {
                    if (component instanceof ActionRow row) {
                        for (final Component column : row.getComponents()) {
                            if (column instanceof SelectMenu selection) {
                                menu = selection;
                                break;
                            }
                        }
                ...
                menu.getOptions().add(
                        SelectOption.of(role.getName(), role.getId()).withEmoji(Emoji.fromFormatted(emoji))
