...
public class PropertyPauseRentSubcommand extends PropertySubcommand {
    public PropertyPauseRentSubcommand() {
        super("pause-rent", "Pause renting a property");
        addOption(OptionType.STRING, "property", "The property to pause renting", true, true);
    }
    ...
    protected void execute(SlashCommandInteractionEvent event, Guild guild, Economy account, GuildData config) {
        ...
        PropertyCommand.hookReply(event, "✅ Rent paused for %s.".formatted(property.getName()));
    }
}
