...
public class HentaiThighCommand extends NSFWCommand {
    public HentaiThighCommand() {
        super(NSFWCategory.FAKE);
    }

    @Override
    public String getDescription() {
        return "Grabs some hentai thigh images and other media";
    }
    ...
    protected void runNormalMessage(MessageReceivedEvent event) {
        ...
        try (var response = Constants.HTTP_CLIENT.newCall(request).execute()) {
            ...
            event.getChannel().sendMessage(
                "There was an issue retrieving some hentai thigh. Please report the following error to the bot owner:\n"
                    + strBuilder.toString())
